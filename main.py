...
from forms import Login, SearchEquipmentForm, ChangePasswordForm, EditInfoForm, SearchStaffForm, NewStoreForm, StoreForm, BorrowForm, RegistrationForm,DeleteForm
...
def change_info():
    form = EditInfoForm()
    if form.validate_on_submit():
        current_user.staff_name = form.name.data
        db.session.add(current_user)
        flash(u'已成功修改个人信息！')
        return redirect(url_for('user_info', id=current_user.card_id))
    form.name.data = current_user.staff_name
...
@app.route('/find_equipment', methods=['POST'])
def find_equipment():
    ...
    for equipment in equipments:
        ...
        item = {'equipmentNo': equipment.equipmentNo,
        ...
        if equipment is None:
            ...
            if len(request.form.get('barcode')) == 0:
                ...
                if exist is not None:
                    ...
                    item.admin = current_user.card_id
    ...
    if form.validate_on_submit():
        ...
        if equipment is None:
            ...
            if len(request.form.get('equipmentNo')) == 0:
                ...
                db.session.commit()
                flash(u'设备删除成功', 'success')
        return redirect(url_for('delete_equipment'))
    return render_template('delete-equipment.html', form=form)
    ...
    if form.validate_on_submit():
        if len(request.form.get('equipmentNo')) == 0:
            ...
            if exist is not None:
                ...
                equipment.pic_path=request.form.get('pic_path')
...
def upload():
    ...
    file_path='./static/equip_image/' + time.strftime("%Y%m%d%H%M%S") + extension
    file.save(file_path)
    j_path = {
        'src': file_path,
    }
    return jsonify(j_path)
